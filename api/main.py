...
from data.indian_symbol_mapper import INDIAN_WATCHLIST, get_market_status
...
@app.get("/symbols/india")
async def symbols_india():
    """Return the preset Indian market watchlist."""
    return {
        "exchange": "NSE/BSE",
        "symbols": INDIAN_WATCHLIST,
    }


@app.get("/market/status", response_model=MarketStatus)
async def market_status(exchange: str = "NSE"):
    """Return the current market status for the given Indian exchange."""
    result = get_market_status(exchange)
    return MarketStatus(**result)


@app.get("/indicators")
async def indicators():
    """Return the list of supported technical indicators."""
    return {
        "indicators": [
            ...
            {"id": "bb",   "name": "Bollinger Bands",             "params": ["period", "std"]},
